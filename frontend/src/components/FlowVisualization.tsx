...
export default function FlowVisualization() {
  ...
  const [layoutMode, setLayoutMode] = useState<'hierarchical' | 'force'>('force')
  ...
  const getNodeColor = useCallback((labels: Record<string, string>, namespace: string, isRisky: boolean): any => {
    ...
    const colors = namespaceColors[namespace] || { bg: '#f9fafb', border: '#6b7280' }
    return {
  ...
  const buildNetwork = useCallback((flows: Flow[]) => {
    ...
    filteredFlows.forEach(flow => {
      ...
      if (!nodesMap.has(dstId)) {
        ...
        const appLabel = flow.dst_labels.app || flow.dst_labels.role || 'unknown'
    ...
    flowGroups.forEach((flowGroup, key) => {
      ...
      // Aggregate ports
      const ports = [...new Set(flowGroup.map(f => `${f.port}/${f.protocol}`))].join(', ')
      ...
      edges.push({
        ...
        title: `From: ${firstFlow.src_ns}/${firstFlow.src_pod}\nTo: ${firstFlow.dst_ns}/${firstFlow.dst_pod}\nPorts: ${ports}\nFlows: ${flowGroup.length}\n${hasRisky ? '⚠️ RISKY' : ''}${hasUnprotected ? '\n🔓 UNPROTECTED' : ''}`,
        metadata: {
          flows: flowGroup,
          is_risky: hasRisky,
    ...
    <div className="flex gap-4" style={{ minHeight: '700px' }}>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
          <div className="flex flex-wrap gap-4 items-end">
            ...
            <div className="min-w-[150px]">
              ...
              <select
                value={layoutMode}
                onChange={(e) => setLayoutMode(e.target.value as 'hierarchical' | 'force')}
                className="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
              >
            ...
            <div className="flex gap-4 text-xs text-gray-600">
              <div>
                <span className="font-semibold text-gray-900">{stats.totalPods}</span> Workloads
              </div>
              <div>
                <span className="font-semibold text-gray-900">{stats.totalFlows}</span> Flows
              </div>
              {stats.riskyFlows > 0 && (
                <div className="text-orange-600">
                  <span className="font-semibold">{stats.riskyFlows}</span> Risky
                </div>
              )}
            </div>
          </div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <div className="space-y-2">
                    <div>
                      ...
                      <div className="text-sm font-semibold text-gray-900">{selectedNodeData.pod}</div>
                    </div>
                    <div>
                      <div className="text-xs text-gray-500">Namespace</div>
                      <div className="text-sm text-gray-900">{selectedNodeData.namespace}</div>
                    </div>
                    <div>
                      <div className="text-xs text-gray-500">Application</div>
                      <div className="text-sm text-gray-900">{selectedNodeData.app || 'N/A'}</div>
                    </div>
                ...
                <div>
                  ...
                  <div className="bg-gray-50 rounded p-2 text-xs font-mono">
                    {Object.entries(selectedNodeData.labels || {}).map(([key, value]) => (
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <div className="space-y-2">
                    <div>
                      ...
                      <div className="text-xs text-gray-500">{selectedEdgeData.flows[0].src_ns}</div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="text-xs text-gray-500">{selectedEdgeData.flows[0].dst_ns}</div>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      ...
                      <span className="font-mono text-gray-900">
                        {[...new Set(selectedEdgeData.flows.map((f: Flow) => `${f.port}/${f.protocol}`))].join(', ')}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600">Flow Count:</span>
                      <span className="font-semibold text-gray-900">{selectedEdgeData.flow_count}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600">Status:</span>
                      <span className={`font-semibold ${
                        selectedEdgeData.flows[0].verdict === 'allow' ? 'text-green-600' : 'text-red-600'
                      }`}>
                ...
                {selectedEdgeData.is_risky && (
                  <div className="px-3 py-2 bg-orange-50 border border-orange-200 rounded">
                    <div className="text-xs font-medium text-orange-800 mb-1">⚠️ Risky Flow</div>
                ...
                {selectedEdgeData.is_unprotected && (
                  <div className="px-3 py-2 bg-yellow-50 border border-yellow-200 rounded">
                    <div className="text-xs font-medium text-yellow-800 mb-1">🔓 Unprotected Flow</div>
      ...
      {!showDetails && (
        <button
          ...
          className="fixed right-4 top-1/2 bg-blue-600 text-white px-4 py-2 rounded-lg shadow-lg hover:bg-blue-700 transition"
        >
          Show Details
        </button>
      )}
